...
const Organization = require('./users/Org.js');
const { xpired } = require('./xpired.js');
const { openMeeting } = require('./middleware/openMeet.js');
...
const host = 'https://room.didar.gwsec.ir';
const jwtCfg = {
...
function startServer() {
    ...
    // join room by id
    app.get('/join/:roomId', openMeeting, async (req, res, next) => {
        try {
            const meeting = await Didar.findById(req.params.roomId);

            if (meeting.isOpen == false) {
                const error = new Error("صبر کنید تا میزبان جلسه وارد شود. بعدا دوباره اقدام به ورود کنید");
    ...
    app.put(['/isopen/:roomId'], async (req, res) => {
        const meeting = await Didar.findById(req.params.roomId);
    ...
    app.post(['/api/user/register'], authenticated2, async (req, res, next) => {
        try {
            ...
            if (req.userType == "admin") {
                ...
                const org = await Organization.findOne({ code: organization_code });
                const organization_name = org.name;
                await User.create({ organization_code, user_type, fullname, username, password, organization_name });
                return res.status(201).json({ message: "عضویت با موفقیت انجام شد" });
            }
            else {
                const error = new Error("شما اجازه ساخت کاربر را ندارید");
                error.statusCode = 403;
                throw error;
            }
    ...
    // Delete User - API
    app.delete(['/api/user/delete/:id'], authenticated2, async (req, res, next) => {
        try {
            if (req.userType == "admin") {
                ...
                const error = new Error("شما اجازه حذف کاربر را ندارید");
                error.statusCode = 403;
                throw error;
            }
        } catch (err) {
    ...
    app.put(['/api/user/change-password'], authenticated2, async (req, res, next) => {
    ...
    app.put(['/api/user/change-password/:id'], authenticated2, async (req, res, next) => {
        try {
            if (req.userType == "admin") {
                const user = await User.findOne({ _id: req.params.id });
                if (!user) {
    ...
    // API GET All of User for admin
    app.get(['/api/user/all'], authenticated2, async (req, res, next) => {
        try {
            if (req.userType == "admin") {
                ...
                if (!users) {
                    ...
                    for (let i = 0; i < users.length; i++) {
                        if (users[i].username == req.username)
                            continue;
                        p.push(lodash.pick(users[i], ["_id", "username", "fullname", "user_type", "organization_code", "organization_name"]));
                    }
    ...
    app.get(['/api/user/partners'], authenticated2, async (req, res, next) => {
        try {
            if (req.userType == "supervisor" || req.userType == "admin") {
                ...
                for (let i = 0; i < partners.length; i++) {
                    if (partners[i].username == req.username)
                        continue;
                    p.push(lodash.pick(partners[i], ["fullname", "username", "organization_name", "organization_code"]));
                }
    ...
    // Create Didar- API
    app.post(['/api/meeting/add'], authenticated2, async (req, res, next) => {
        try {
            if (req.userType == "supervisor" || req.userType == "admin") {
    ...
    app.get(['/api/meeting/all'], authenticated2, async (req, res, next) => {
        try {
            const allDidar = await Didar.find();
            var date;
            for (let i = 0; i < allDidar.length; i++) {
                date = allDidar[i].date.split('/');
                if (xpired(date) == true) {
                    await Didar.findByIdAndDelete(allDidar[i]._id);
                }
            }
            const didar = await Didar.find({ presenter: req.username });
            const didar2 = await Didar.find({ invited: req.username });
    ...
    // Delete Didar
    app.delete(['/api/meeting/delete/:id'], authenticated2, async (req, res, next) => {
        try {
            if (req.userType == "supervisor" || req.userType == "admin") {
                await Didar.findByIdAndDelete(req.params.id);
                res.status(200).json({ message: "جلسه با موفقیت حذف شد" })
            } else {
                const error = new Error("شما دسترسی ندارید");
                error.statusCode = 403;
                throw error;
    ...
    // create Org
    app.post(['/api/org/add'], authenticated2, async (req, res, next) => {
        try {
            if (req.userType == "admin" || req.userType == "supervisor") {
                const { code, name, parent } = req.body;
                const org = await Organization.findOne({ code });
                const org2 = await Organization.findOne({ code: parent });
                if (org) {
                    const error = new Error("سازمانی با این کد موجود است");
                    error.statusCode = 422;
                    throw error;
                }
                if (!org2) {
                    const error = new Error("والدی با این کد یافت نشد. مطمئن شوید کد والد درست باشد");
                    error.statusCode = 404;
                    throw error;
                }
                ...
                res.status(200).json({ message: "سازمان با موفقیت ساخته شد" });
            }
            else {
                const error = new Error("شما دسترسی ندارید");
                error.statusCode = 403;
    ...
    // get all Org
    app.get(['/api/org/all'], authenticated2, async (req, res, next) => {
        try {
            if (req.userType == "admin" || req.userType == "supervisor") {
                const organizations = await Organization.find();
                if (organizations.lengh == 0) {
                    const error = new Error("هیچ سازمانی ثبت نشده است");
                    error.statusCode = 404;
                    throw error;
                }
    ...
    // Delete Org
    app.delete(['/api/org/delete/:id'], authenticated2, async (req, res, next) => {
        try {
            if (req.userType == "admin") {
                await Organization.findByIdAndDelete(req.params.id);
                res.status(200).json({ message: "سازمان با موفقیت حذف شد" })
            } else {
                const error = new Error("شما دسترسی ندارید");
                error.statusCode = 403;
                throw error;
    ...
    app.put(['/api/org/edit/:id'], authenticated2, async (req, res, next) => {
        try {
            if (req.userType == "admin") {
                const { code, name } = req.body;
                const org = await Organization.findById(req.params.id);
                if (!org) {
                    const error = new Error("هیچ سازمانی با این آیدی یافت نشد");
                    error.statusCode = 404;
                    throw error;
                } else if(!code || !name){
                    const error = new Error("کد و نام الزامی است");
                    ...
                    org.name = name;
                    await org.save();
                    res.status(200).json({ message: "سازمان با موفقیت ویرایش شد" })
                }
    ...
    io.on('connection', (socket) => {
        ...
        async function removeMeData(room, peerName, isPresenter) {
            const roomId = room && socket.room_id;
            const peerCounts = room && room.getPeers().size;
            const meeting = await Didar.findById(roomId);
            if (isPresenter == true) {
                meeting.isOpen = false;
                await meeting.save();
            }
